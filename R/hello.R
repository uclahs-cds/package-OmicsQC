source("/Users/hugofrelin/Desktop/Boutros Lab/template-R-package/R/000_HEADER.R")
...
# # hg38 QC
# print(load("Summary_Tables/hg38.QC.rsav"))
# # [1] "qc_data"
# 
# # Correlation with RNA-seq
# bulk = read.table("Summary_Tables/rna_cor/2020-09-15/2020-09-15_m6a_input_vs_bulk_per_sample.txt", header = T)
# colnames(bulk) = c("Sample", "Bulk.Rho", "pvalue")
# 
# microarray = read.table("Summary_Tables/rna_cor/2020-09-15/2020-09-15_m6a_input_vs_array_per_sample.txt", header = T)
# colnames(microarray) = c("Sample", "Array.Rho", "pvalue")
# 
# # Merging data
# qc_data = merge(qc_data, bulk[,c("Sample", "Bulk.Rho")], by = "Sample", all.x = T)
# qc_data$Bulk.Rho[is.na(qc_data$Bulk.Rho)] <- mean(qc_data$Bulk.Rho, na.rm = T)
# qc_data = merge(qc_data, microarray[,c("Sample", "Array.Rho")], by = "Sample", all.x = T)
# qc_data$Array.Rho[is.na(qc_data$Array.Rho)] <- mean(qc_data$Array.Rho, na.rm = T)
# 
# filename = "Summary_Tables/QC_compiled_for_Hugo.tsv"
# write.table(
#   qc_data,
#   file = filename,
#   col.names = T,
